...
// ── Dashboard ────────────────────────────────────────────────────────────
function Dashboard({ copy, profile, portfolio, density, mode = 'initial', pendingOrders = [], onAddOrder, onRemoveOrder, onAuthorize, onBuy, onCounsel }) {
  ...
  return (
    <div style={{ height: '100%', background: 'var(--bg)', color: 'var(--ink)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ borderBottom: '2px solid var(--ink)', padding: '20px 40px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        ...
        <div style={{ display: 'flex', gap: 24, fontFamily: 'JetBrains Mono, monospace', fontSize: 10, color: 'var(--ink-mute)', letterSpacing: '0.12em', textTransform: 'uppercase' }}>
          <span>Portfolio</span><span>Counsel</span><span>Activity</span><span>Settings</span>
        </div>
      </div>

      {/* Hero */}
      <div style={{ padding: '28px 40px 20px', borderBottom: '1px solid var(--rule)', display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 32, alignItems: 'flex-end' }}>
        <div>
          ...
          <section>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 4 }}>
                {['1M', '6M', '1Y', '5Y', 'Max'].map((r, i) => (
                  <button key={r} style={{
                    fontFamily: 'JetBrains Mono, monospace', fontSize: 10,
                    padding: '4px 10px', background: i === 4 ? 'var(--ink)' : 'transparent',
                    color: i === 4 ? 'var(--bg)' : 'var(--ink-soft)',
            ...
            <div style={{ height: 220, border: '1px solid var(--rule)', padding: '12px 4px 0 12px', background: 'var(--surface)' }}>
              <AreaChart data={portfolio.series} height={208} accent="var(--accent)"
                yLabels={[
                  portfolio.budget * 0.9,
                  portfolio.budget,
                  portfolio.budget * 1.2,
                  portfolio.budget * 1.5,
                ]}
                xLabels={['May ’26', 'Nov ’26', 'May ’27', 'Nov ’27', 'Apr ’28']} />
            </div>
...
function Counsel({ copy, profile, portfolio, automation, setAutomation, onBack }) {
                        ...
                        <div key={ai} style={{
                          ...
                          <div style={{ paddingTop: 4 }}>
                            {on ? (
                              ...
                              <Button variant="ghost" style={{ padding: '5px 12px', fontSize: 11 }}>Schedule</Button>
                            )}
                          </div>
                        </div>
                      ))}
